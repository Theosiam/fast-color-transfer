function [ssimval, ssimmap] = gssim(varargin)
...
[A, B, C, exponents, radius] = parse_inputs(args{:});
...
if isa(A,'int16') % int16 is the only allowed signed-integer type for A and ref.
    ...
    B = double(B) - double(intmin('int16'));
elseif isinteger(A)
    A = double(A);
    B = double(B);
end
...
AF=double(sobel_filter(A));
BF=double(sobel_filter(B));

mux2 = gaussFilterFcn(A);
muy2 = gaussFilterFcn(B);
muxy = mux2.*muy2;
mux2 = mux2.^2;
muy2 = muy2.^2;


Fmux2 = gaussFilterFcn(AF);
Fmuy2 = gaussFilterFcn(BF);
Fmuxy = Fmux2.*Fmuy2;
Fmux2 = Fmux2.^2;
Fmuy2 = Fmuy2.^2;
% Clamp to zero. Floating point math sometimes makes this negative
sigmax2 = max(gaussFilterFcn(AF.^2) - Fmux2,0);
sigmay2 = max(gaussFilterFcn(BF.^2) - Fmuy2,0);

sigmaxy = gaussFilterFcn(AF.*BF) - Fmuxy;
